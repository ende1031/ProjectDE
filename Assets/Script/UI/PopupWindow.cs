    ...
    public void AddItem(Inventory.Item itemName) //아이템 추가시 수정할 부분
    {
        ...
        switch (itemName)
        {
            case Inventory.Item.Trap01:
                WindowItemList.Add(new WindowItem(itemName, 30, "소형 덫", "가시 덩굴의 가시로 만든 덫이다.\n괴물의 둥지 근처에 설치해두면 괴물의 심장을 얻을 수 있다." + TimeToString(30), Inventory.Item.Stick, 1, Inventory.Item.Thorn, 2));
                break;
            ...
            case Inventory.Item.Facility01:
                WindowItemList.Add(new WindowItem(itemName, 30, "소형 워크벤치", "괴물의 조직을 이용해서 만든 워크벤치이다. 살아있지만 위험하지는 않다.\n각종 아이템을 제작할 수 있다." + TimeToString(30), Inventory.Item.Stick, 2, Inventory.Item.Board, 1, Inventory.Item.Heart, 1));
                break;
            case Inventory.Item.Bulb01:
                WindowItemList.Add(new WindowItem(itemName, 30, "간이 전구", "빛을 내서 괴물의 접근을 막는 시설이다.\n간이 전구의 양 옆 2칸 이내에는 다른 시설을 설치할 수 있다." + TimeToString(30), Inventory.Item.Mass, 1, Inventory.Item.Heart, 1, Inventory.Item.Hose, 1, Inventory.Item.Tumor, 1));
                break;
            case Inventory.Item.Food:
                WindowItemList.Add(new WindowItem(itemName, 45, "식량", "유전자 조작으로 만든 괴물종양을 가공해서 식품으로 만들었다.\n젠장 맛없다.\n사용하면 허기 게이지를 충전할 수 있다." + TimeToString(45), Inventory.Item.Tumor, 2));
                break;
            case Inventory.Item.StickSeed:
                WindowItemList.Add(new WindowItem(itemName, 40, "집게발 대나무 모종", "심으면 긴 막대처럼 생긴 괴식물이 자라난다." + TimeToString(40), Inventory.Item.Stick, 3, Inventory.Item.Mass, 1, Inventory.Item.Heart, 1));
                break;
            case Inventory.Item.BoardSeed:
                WindowItemList.Add(new WindowItem(itemName, 40, "판자 식물 모종", "심으면 판자처럼 생긴 괴식물이 자라난다." + TimeToString(40), Inventory.Item.Board, 1, Inventory.Item.Mass, 1, Inventory.Item.Heart, 1));
                break;
            case Inventory.Item.ThornSeed:
                WindowItemList.Add(new WindowItem(itemName, 40, "가시 덩굴 모종", "심으면 가시가 달린 괴식물이 자라난다." + TimeToString(40), Inventory.Item.Thorn, 1, Inventory.Item.Mass, 1, Inventory.Item.Heart, 1));
                break;
            case Inventory.Item.Oxygen:
                WindowItemList.Add(new WindowItem(itemName, 50, "산소", "폐허가 된 지구에서 숨을 쉬기 위해서는 정제를 통해 산소를 얻는 방법 밖에는 없다.\n산소가 다 떨어지면 나는 어떻게 될까?\n사용하면 산소 게이지를 충전할 수 있다." + TimeToString(50), Inventory.Item.Water, 3));
                break;
            case Inventory.Item.TumorSeed:
                WindowItemList.Add(new WindowItem(itemName, 30, "종양 씨앗", "유전자 조작으로 만든 안전한 종양 씨앗이다.\n식물에 심으면 식용 종양을 얻을 수 있다." + TimeToString(30), Inventory.Item.Mass, 3));
                break;
            case Inventory.Item.Grinder01:
                WindowItemList.Add(new WindowItem(itemName, 30, "분쇄기", "필요 없는 아이템을 분해해서 다른 아이템을 얻는데 사용하는 시설이다." + TimeToString(30), Inventory.Item.Stick, 1, Inventory.Item.Board, 5, Inventory.Item.Thorn, 2, Inventory.Item.Hose, 2, Inventory.Item.Mass, 1, Inventory.Item.Heart, 1));
                break;
        }
    }
