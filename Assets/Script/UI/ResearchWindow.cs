...
public class ResearchWindow : MonoBehaviour
{
    ...
    {
        public ResearchItem(string i, int m, string t)
        {
            item = i;
            maxNum = m;
            expText = t;
    ...
    public class ResultContent
    {
        ...
        public ResultContent(string t, Sprite sp, string it)
        {
            expText = t;
            ContentSp = sp;
            item = it;
    ...
    {
        //덩어리 3개
        itemArray[0] = new ResearchItem("Item_Mass", 3, "연구를 통해 닉스입자를 발견했다.\n연구 성과로 닉스입자를 획득했다! 닉스입자는 화면 상단 UI에서 확인할 수 있다.");
        itemArray[0].SetResultItem(1, "Nyx");
        itemArray[0].AddNextResearch(1);
        //사과 2개
        itemArray[1] = new ResearchItem("Item_Fruit", 2, "이번 연구는 먹을 수 있는 것을 발견했다는 점에 큰 의의가 있다. 구조가 오기 전 까지 무사히 이 행성에서 살아남을 수 있을 것 같다.");
        itemArray[1].SetResultItem(2, "FruitPlant", "Hunger");
        itemArray[1].AddNextResearch(2);
        //가시 3개
        itemArray[2] = new ResearchItem("Item_Thorn", 5, "선인장의 변종으로 예상했으나 가시의 성분은 식물의 조직보다는 동물의 뼈나 뿔에 가깝다는걸 알게 됐다.");
        itemArray[2].SetResultItem(3, "ThornPlant", "Trap01", "Capture");
        itemArray[2].AddNextResearch(3);
        //심장 1개
        itemArray[3] = new ResearchItem("Item_Heart", 1, "분명 이 행성에도 살아있는 생명체가 존재한다. 하지만 조사하기 위해 감수해야 하는 위험이 너무 크다.");
        itemArray[3].SetResultItem(1, "Heart");
        itemArray[3].AddNextResearch(4);
        //막대 3개
        itemArray[4] = new ResearchItem("Item_Stick", 6, "닉스입자에 대한 연구가 거의 완성되었다. 하지만 아직 부족하다. 다음 연구를 진행해보자.");
        itemArray[4].SetResultItem(1, "StickPlant");
        itemArray[4].AddNextResearch(5);
        //판자 3개
        itemArray[5] = new ResearchItem("Item_Board", 3, "드디어 닉스입자를 수집하는 방법을 알게 됐다. 닉스입자를 이용해서 무언가를 만들 수 있을 것 같다.");
        itemArray[5].SetResultItem(2, "BoardPlant", "NyxCollector01");
        itemArray[5].AddNextResearch(6);
        //심장 4개
        itemArray[6] = new ResearchItem("Item_Heart", 3, "모든 제작을 탈출포드에서 해야 된다는 것은 지나치게 비효율적이다. 괴식물의 심장을 이용해서 다른 제작 시설을 만들어보기로 했다.");
        itemArray[6].SetResultItem(1, "TempFacility");
        itemArray[6].AddNextResearch(7);
        //막대 10개
        itemArray[7] = new ResearchItem("Item_Stick", 10, "괴물은 빛을 싫어한다. 빛을 내는 시설을 만들 수 있으면 다른 시설들을 안전하게 지킬 수 있을 것이다.");
        itemArray[7].SetResultItem(2, "Hose", "Bulb01");
        itemArray[7].AddNextResearch(8);
        //판자 10개
        itemArray[8] = new ResearchItem("Item_Board", 10, "연구를 통해 워크벤치에서 분해기를 만들 수 있게 됐다.\n이제 필요없는 아이템은 분해하도록 하자.");
        itemArray[8].SetResultItem(2, "Sawtooth", "Grinder01");
        itemArray[8].AddNextResearch(9);
        itemArray[8].AddNextResearch(10);
        //덩어리 20개
        itemArray[9] = new ResearchItem("Item_Mass", 20, "괴식물의 유전자를 조작해서 먹을 수 있는 안전한 조직을 만들어냈다. 자라는 중인 식물에 심어보자.");
        itemArray[9].SetResultItem(3, "TumorSeed", "SeedingTumor", "Food");
        itemArray[9].AddNextResearch(9);
        //막대 20개
        itemArray[10] = new ResearchItem("Item_Stick", 20, "집게발 대나무의 유전자를 조작해서 모종을 만들 수 있게 됐다. 괴식물의 모종을 만들면 원하는 위치에 심을 수 있다.");
        ...
        //판자 20개
        itemArray[11] = new ResearchItem("Item_Board", 20, "판자 식물의 유전자를 조작해서 모종을 만들 수 있게 됐다. 괴식물의 모종을 만들면 원하는 위치에 심을 수 있다.");
        itemArray[11].SetResultItem(1, "BoardSeed");
        itemArray[11].AddNextResearch(11);
        //가시 20개
        itemArray[12] = new ResearchItem("Item_Thorn", 20, "가시 덩굴의 유전자를 조작해서 모종을 만들 수 있게 됐다. 괴식물의 모종을 만들면 원하는 위치에 심을 수 있다.");
        itemArray[12].SetResultItem(1, "ThornSeed");
        itemArray[12].AddNextResearch(12);
        //사과 20개
        itemArray[13] = new ResearchItem("Item_Fruit", 20, "열매 나무의 유전자를 조작해서 모종을 만들 수 있게 됐다. 괴식물의 모종을 만들면 원하는 위치에 심을 수 있다.");
        itemArray[13].SetResultItem(1, "FruitSeed");
        itemArray[13].AddNextResearch(13);
        //호스 30개
        itemArray[14] = new ResearchItem("Item_Hose", 30, "집으로 돌아가고 싶다. 참치마요가 먹고싶다. 빨리 구조대가 왔으면 좋겠다. 연구는 순조롭다. 이 행정의 진실에 근접했다고 확언할 수 있다.");
        itemArray[14].SetResultItem(1, "Earth0");
        itemArray[14].AddNextResearch(15);
        //톱날 30개
        itemArray[15] = new ResearchItem("Item_Sawtooth", 30, "마지막 연구를 끝냈다. 이 행성은 멸망 이후 독자적인 생태계를 만들어가고 있다. 이렇게 된 원인은 바로 ··· ···");
    ...
    {
        contentDictionary["Nyx"] = new ResultContent("[닉스 입자] 이 행성에만 존재하는 미지의 입자로 추청된다. 무언가를 제작하는데 사용할 수 있을 것 같다.\n연구성과 : 닉스입자 +240", inventory.GetItemSprite("Item_Nyx"));
        contentDictionary["FruitPlant"] = new ResultContent("[괴식물의 열매] 인체에 해롭지 않아서 먹을 수 있다. 허기를 소량 회복시켜준다.", inventory.GetItemSprite("Item_Fruit"));
        contentDictionary["Hunger"] = new ResultContent("[허기 회복] 먹을 수 있는 아이템을 먹으면 허기게이지를 회복시킬 수 있다. 게이지가 바닥나서 죽지 않게 조심하자.", inventory.GetItemSprite("Item_Hunger"));
        contentDictionary["ThornPlant"] = new ResultContent("[가시 덩굴] 가시가 자라는 나무이다. 가시 덩굴에서 채집한 가시는 소형 덫을 만드는데 사용할 수 있다.", inventory.GetItemSprite("Item_Thorn"));
        contentDictionary["Trap01"] = new ResultContent("[소형 덫] 소형 덫을 만들 수 있게 됐다.\n괴물의 둥지 근처에 설치하면 괴물의 심장을 얻을 수 있다.", inventory.GetItemSprite("Item_Trap01"), "Item_Trap01");
        contentDictionary["Capture"] = new ResultContent("[포획] 괴물의 둥지 근처에 덫을 설치하면 괴물을 잡을 수 있다. 설치 후 날짜가 지나면 괴물이 잡힌다.", inventory.GetItemSprite("Item_Capture"));
        contentDictionary["Heart"] = new ResultContent("[심장] 작은 괴물의 심장이다. 계속해서 에너지를 발생시키고 있다. 시설을 만드는데 사용할 수 있다.", inventory.GetItemSprite("Item_Heart"));
        contentDictionary["StickPlant"] = new ResultContent("[집게발 대나무] 대나무를 닮은 괴식물이다. 집게발 대나무는 하루에 한번씩 채집할 수 있다.", inventory.GetItemSprite("Item_Stick"));
        contentDictionary["BoardPlant"] = new ResultContent("[판자 식물] 판자를 닮은 괴식물이다. 현실 시간으로 1분에 한번씩 채집할 수 있다. 판자는 제작에 사용된다.", inventory.GetItemSprite("Item_Board"));
        contentDictionary["NyxCollector01"] = new ResultContent("[닉스입자 수집기] 닉스입자 연구가 끝나서 닉스입자 수집기를 만들 수 있게 됐다.", inventory.GetItemSprite("Item_NyxCollector01"), "Item_NyxCollector01");
        contentDictionary["TempFacility"] = new ResultContent("[소형 워크벤치] 소형 워크벤치를 만들 수 있게 됐다.", inventory.GetItemSprite("Item_Facility01"), "Item_Facility01");
        contentDictionary["Hose"] = new ResultContent("[호스] 호스를 만들 수 있게 됐다.\n호스는 간이 전구같은 시설을 만드는데 사용된다.", inventory.GetItemSprite("Item_Hose"), "Item_Hose");
        contentDictionary["Bulb01"] = new ResultContent("[간이 전구] 간이 전구를 만들 수 있게 됐다. 간이 전구 근처에는 다른 시설을 설치할 수 있다.", inventory.GetItemSprite("Item_Bulb"), "Item_Bulb");
        contentDictionary["Sawtooth"] = new ResultContent("[톱날] 톱날을 만들 수 있게 됐다.\n톱날은 분해기를 만드는데 사용된다.", inventory.GetItemSprite("Item_Sawtooth"), "Item_Sawtooth");
        contentDictionary["Grinder01"] = new ResultContent("[간이 분해기] 간이 분해기를 만들 수 있게 됐다. 불필요한 아이템을 분해해서 닉스입자와 다른 아이템을 얻을 수 있다.", inventory.GetItemSprite("Item_Grinder01"), "Item_Grinder01");
        contentDictionary["TumorSeed"] = new ResultContent("[종양 씨앗] 종양 씨앗을 만들 수 있게 됐다. 자라는 중인 식물에 심으면 먹을 수 있는 종양이 생긴다.", inventory.GetItemSprite("Item_TumorSeed"), "Item_TumorSeed");
        contentDictionary["SeedingTumor"] = new ResultContent("[종양 심기] 자라는중인 특정 식물에는 종양을 심을 수 있다. 집게발 대나무, 판자 식물, 가시 덩굴에 심을 수 있다.", inventory.GetItemSprite("Item_SeedingPlant"));
        contentDictionary["Food"] = new ResultContent("[식량] 식량을 만들 수 있게 됐다. 식량은 한번에 많은 양의 게이지를 회복시켜준다.", inventory.GetItemSprite("Item_Food"), "Item_Food");
        contentDictionary["StickSeed"] = new ResultContent("[집게발 대나무 모종] 집게발 대나무 모종을 만들 수 있게 됐다.", inventory.GetItemSprite("Item_StickSeed"), "Item_StickSeed");
        contentDictionary["SeedingPlant"] = new ResultContent("[모종 심기] 원하는 위치에 모종을 심으면 괴식물이 자란다.", inventory.GetItemSprite("Item_SeedingPlant"));
        contentDictionary["BoardSeed"] = new ResultContent("[판자 식물 모종] 판자 식물 모종을 만들 수 있게 됐다.", inventory.GetItemSprite("Item_BoardSeed"), "Item_BoardSeed");
        contentDictionary["ThornSeed"] = new ResultContent("[가시 덩굴 모종] 가시 덩굴 모종을 만들 수 있게 됐다.", inventory.GetItemSprite("Item_ThornSeed"), "Item_ThornSeed");
        contentDictionary["FruitSeed"] = new ResultContent("[열매 나무 모종] 열매 나무 모종을 만들 수 있게 됐다.", inventory.GetItemSprite("Item_FruitSeed"), "Item_FruitSeed");
        contentDictionary["Earth0"] = new ResultContent("[파괴된 행성] 이 행성에 대한 연구는 순조롭게 진행되고 있다. 다음 연구가 마지막 연구일 것이다.", inventory.GetItemSprite("Item_Earth0"));
        contentDictionary["Earth1"] = new ResultContent("[행성의 생태계] 빛을 싫어하는 미지의 생명체에 대한 생태계 연구 보고서. 본부에 귀환해서 발표하면 좋은 평가를 받을 것 같다.", inventory.GetItemSprite("Item_Earth1"));
        contentDictionary["Earth2"] = new ResultContent("[멸망한 고대 문명] 이 행성에 존재했던 고대문명에 대한 연구 보고서. 이 사실이 알려지면 큰 파장을 불러올 것으로 예상된다.", inventory.GetItemSprite("Item_Earth2"));
    }
    ...
    void RefreshWindow()
    {
        ...
        else
        {
            ...
            BigItem.GetComponent<Image>().sprite = inventory.GetItemSprite(itemArray[selectedIndex].item);
        ...
        for (int i = 0; i < 16; i++)
        {
            ...
            else
            {
                ...
                ItemIcon[i].GetComponent<Image>().sprite = inventory.GetItemSprite(itemArray[i].item);
                ItemIconBack[i].GetComponent<Image>().sprite = inventory.GetItemSprite(itemArray[i].item);
    ...
    void OpenResultWindow()
    {
        ...
        ResultBigItem.GetComponent<Image>().sprite = inventory.GetItemSprite(itemArray[selectedIndex].item);
